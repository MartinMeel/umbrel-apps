...
function renderShares(data) {
  shares.innerHTML = data.shares
    .map(
      (entry) =>
        `<article class="card"><div class="card-top"><strong>${safe(entry.share)}</strong>${badge(entry.mounted, entry.mounted ? "Mounted" : "Unmounted")}</div>${renderKeyValue([{ label: "Target", value: safe(entry.mountpoint) }, { label: "Directory exists", value: yesNo(entry.exists) }, { label: "Really mounted", value: yesNo(entry.mounted) }])}</article>`,
    )
    .join("");
}
