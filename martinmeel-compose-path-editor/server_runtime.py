...
def run_umbreld_app_command(app_id, action):
  ...
  # app_id is regex-validated above, so it is safe to place in this shell command.
  base_command = '/opt/umbreld/umbreld client ' + subcommand + ' --appId ' + app_id
  # Run as the host's umbrel user. This matches the command that works manually
  # on the Umbrel host and avoids running the Node/tsx client as root.
  host_shell_command = (
  ...
  su_shell_command = (
      'export PATH="' + host_path + '"; '
      'export TMPDIR=/tmp; '
      '/bin/su - umbrel -c ' + json.dumps(base_command)
  )
  candidates = [
      ['/usr/sbin/chroot', '/hostfs', '/bin/sh', '-lc', su_shell_command],
      ['chroot', '/hostfs', '/bin/sh', '-lc', su_shell_command],
  ...
  for cmd in candidates:
      try:
          completed = subprocess.run(cmd, cwd='/', env=env, text=True, capture_output=True, timeout=60, stdin=subprocess.DEVNULL)
      ...
      output = (stderr + '\n' + stdout).strip()
      errors.append('command failed: ' + ' '.join(cmd) + ' :: ' + output)
