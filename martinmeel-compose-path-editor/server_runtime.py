...
def html():
    ...
    return """<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Compose Path Editor v0.4.2</title><style>
:root{--bg1:#0f172a;--bg2:#312e81;--card:#111827cc;--line:#475569;--text:#f8fafc;--muted:#cbd5e1;--blue:#38bdf8;--purple:#a78bfa;--pink:#f472b6;--green:#34d399;--amber:#fbbf24;--red:#fb7185}*{box-sizing:border-box}body{font-family:system-ui,-apple-system,Segoe UI,sans-serif;margin:0;min-height:100vh;background:radial-gradient(circle at 12% 10%,#2563eb55 0,transparent 28%),radial-gradient(circle at 90% 0,#ec489955 0,transparent 26%),linear-gradient(135deg,var(--bg1),#111827 45%,var(--bg2));color:var(--text)}main{max-width:1120px;margin:auto;padding:30px 24px}h1{font-size:40px;margin:0 0 8px;background:linear-gradient(90deg,#67e8f9,#a78bfa,#f472b6,#fbbf24);-webkit-background-clip:text;background-clip:text;color:transparent}h2{margin-top:0}.card{position:relative;background:linear-gradient(145deg,#1f2937dd,#0f172add);border:1px solid #64748b66;border-radius:24px;padding:24px;margin:20px 0;box-shadow:0 20px 60px #0007, inset 0 1px 0 #ffffff14;overflow:hidden}.card:before{content:"";position:absolute;inset:0 0 auto 0;height:4px;background:linear-gradient(90deg,var(--blue),var(--purple),var(--pink),var(--amber));opacity:.95}.card:nth-of-type(1):before{background:linear-gradient(90deg,#38bdf8,#22c55e)}.card:nth-of-type(2):before{background:linear-gradient(90deg,#a78bfa,#38bdf8)}.card:nth-of-type(3):before{background:linear-gradient(90deg,#f472b6,#a78bfa,#38bdf8)}.card:nth-of-type(4):before{background:linear-gradient(90deg,#fbbf24,#fb7185)}select,button{width:100%;font:inherit;border-radius:14px;border:1px solid #64748b99;background:#020617cc;color:#fff;padding:13px 14px;outline:none}select:focus,.box:has(input:focus){border-color:#38bdf8;box-shadow:0 0 0 4px #38bdf833}button{background:linear-gradient(90deg,#2563eb,#7c3aed,#db2777);border:0;font-weight:800;margin-top:16px;box-shadow:0 12px 26px #1d4ed855;cursor:pointer;transition:transform .12s ease,filter .12s ease}button:hover{transform:translateY(-1px);filter:brightness(1.08)}button:active{transform:translateY(0)}.box{display:block;padding:14px;border:1px solid #64748b77;border-radius:15px;background:linear-gradient(135deg,#0f172acc,#111827cc);margin:10px 0;transition:border-color .12s ease,background .12s ease,transform .12s ease}.box:hover{border-color:#38bdf8aa;background:linear-gradient(135deg,#172554cc,#111827cc);transform:translateX(2px)}.box input{accent-color:#38bdf8;transform:scale(1.15);margin-right:9px}.muted{color:var(--muted)}pre{white-space:pre-wrap;background:linear-gradient(135deg,#020617,#0f172a);border:1px solid #334155;border-radius:16px;padding:16px;overflow:auto;color:#dbeafe}.ok{color:#86efac;font-weight:700}.err{color:#fca5a5;font-weight:700}b{color:#e0f2fe}
</style></head><body><main><h1>Compose Path Editor</h1><p class="muted"><b>v0.4.2</b> — Select an installed Umbrel app, choose a volume line, then replace, add, or restore backups. The target app is stopped first and restarted after verified changes.</p><div class="card"><h2>1. Select docker-compose.yml</h2><select id="apps"></select></div><div class="card"><h2>2. Select volume line from docker-compose.yml</h2><select id="lines"></select><pre id="preview">Loading...</pre></div><div class="card"><h2>3. Choose action and volume mapping(s) — v0.4.2 list</h2><label class="box"><input type="radio" name="action" value="replace" checked> <b>Replace only</b><br><span class="muted">Replace the selected source. Select exactly one volume mapping.</span></label><label class="box"><input type="radio" name="action" value="add"> <b>Add only</b><br><span class="muted">Keep the selected line and add one or more new volume lines after it.</span></label><label class="box"><input type="radio" name="action" value="both"> <b>Replace and add</b><br><span class="muted">Replace with first selected mapping and add remaining selected mappings.</span></label><div id="paths"></div><button id="apply">Apply selected action</button><p id="status" class="muted"></p></div><div class="card"><h2>4. Restore a backup</h2><select id="backups"></select><button id="restore">Restore selected backup</button><p id="restoreStatus" class="muted"></p></div><div class="card"><h2>Live debug log</h2><pre id="log">No operation yet.</pre></div></main><script>
