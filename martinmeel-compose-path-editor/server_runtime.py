...
def run_umbreld_app_command(app_id, action):
  ...
  candidates = [
      ['umbreld', 'client', subcommand, '--appId', app_id],
      ['/opt/umbreld/bin/umbreld', 'client', subcommand, '--appId', app_id],
      ['/opt/umbreld/umbreld', 'client', subcommand, '--appId', app_id],
  ]
  env = os.environ.copy()
  env['PATH'] = '/opt/umbreld/bin:/opt/umbreld:/usr/local/bin:/usr/bin:/bin:' + env.get('PATH', '')
  errors = []
  for cmd in candidates:
      try:
          completed = subprocess.run(cmd, cwd='/', env=env, text=True, capture_output=True, timeout=120)
      ...
      errors.append('command failed: ' + ' '.join(cmd) + ' :: ' + ((completed.stderr or completed.stdout or '').strip()))
  raise RuntimeError('Could not run umbreld client apps.%s.mutate for %s. Tried: %s' % (action, app_id, ' | '.join(errors)))
...
def run_with_target_app_restart(app_id, change_func):
  ...
  start_error = None
  try:
      start_result = run_umbreld_app_command(app_id, 'start')
  except Exception as e:
      start_error = e
  ...
  if change_error is not None:
      ...
      if start_error is not None:
          msg += ' Also failed to start the target app again: %s' % start_error
      else:
          msg += ' The target app was started again.'
      raise RuntimeError(msg)
  if start_error is not None:
      raise RuntimeError('Compose change was written and verified, but starting the target app failed: %s. Backup: %s' % (start_error, change_result.get('backup', 'unknown')))

  change_result['app_control'] = {
      'stopped': True,
      'started': True,
      'stop_command': stop_result.get('command'),
      'start_command': start_result.get('command'),
  }
  change_result['verified_message'] = (change_result.get('verified_message') or 'Verified.') + ' The target app was stopped before writing and started again after verification.'
