...
def run_umbreld_app_command(app_id, action):
  ...
  candidates = []
  if Path('/hostfs/opt/umbreld/umbreld').exists():
      candidates.extend([
          ['/usr/sbin/chroot', '/hostfs', '/usr/bin/env', 'PATH=' + host_path, '/opt/umbreld/umbreld', 'client', subcommand, '--appId', app_id],
          ['chroot', '/hostfs', '/usr/bin/env', 'PATH=' + host_path, '/opt/umbreld/umbreld', 'client', subcommand, '--appId', app_id],
      ])
  candidates.extend([
  ...
  for cmd in candidates:
      try:
          completed = subprocess.run(cmd, cwd='/', env=env, text=True, capture_output=True, timeout=180)
